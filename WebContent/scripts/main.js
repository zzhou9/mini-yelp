(function() {
  ...
  function init() {
    ...
    document.querySelector('#login-btn').addEventListener('click', login); //click执行login func
  ...
  function validateSession() {
    ...
    ajax('GET', url, req,
      ...
      function(res) {
        var result = JSON.parse(res); // String --> JSON Obj or array
  ...
  function login() {
    ...
    ajax('POST', url, req,
      ...
      function(res) {
        ...
        if (result.status === 'OK') {
          onSessionValid(result);  //login success, execute onSessionValid
        }
      },
  ...
  function register() {
    ...
    if (username === "" || password == "" || firstName === "" || lastName === "") {
    	// ===当两边数据类型一样，判断；两边数据类型不一样，直接返回false
